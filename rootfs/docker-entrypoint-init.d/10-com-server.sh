...
SRV_CONF_FILE="/etc/apache2/conf.d/z-ocsinventory-server.conf"
...
if [ ! -f ${SRV_CONF_FILE} ]; then
        echo -n "Data will be unpacked...                "
        tar xzf /apache-conf-dir.tgz --backup=simple --suffix=.before-control
        echo [DONE]
