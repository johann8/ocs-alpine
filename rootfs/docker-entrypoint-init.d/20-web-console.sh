...
REPORTS_CONF_FILE="/etc/apache2/conf.d/ocsinventory-reports.conf"
...
# Permissions
echo -n "Setting apache user permissions...      "
chown -R $APACHE_RUN_USER: $OCS_VARLIB_DIR
chown -R $APACHE_RUN_USER: $OCS_WEBCONSOLE_DIR
echo [DONE]

# rm install.php
if [ -f $OCS_WEBCONSOLE_DIR/ocsreports/install.php ] && [ "${OCS_INVENTOTRY_INSTALL}" == 'false' ]; then
	echo -n "Removing file \"install.php\"...          "
